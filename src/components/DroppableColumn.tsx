...
const DroppableColumn: React.FC<DroppableColumnProps> = ({ id, title, tasks, onTaskSelect, onCreateTask }) => {
  ...
  return (
    <div ref={setNodeRef} className={`${bgColor} ${hoverClass} p-4 rounded-lg border ${borderColor} min-h-[12rem] transition-colors duration-200`} onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
      <div className="flex items-center mb-3">
        <span className={`w-3 h-3 ${dotColor} rounded-full mr-2`}></span>
        <h2 className="font-bold">{title}</h2>
        <span className="ml-2 text-xs bg-white/50 px-2 py-1 rounded-full">{tasks.length}</span>
      </div>

      <div className={`space-y-3 min-h-[8rem] ${isOver ? 'bg-white/30 rounded-lg p-2' : ''} relative`}>
        {tasks.map((task) => (
          <TaskComponent key={task.id} task={task} onSelect={onTaskSelect} />
        ))}
        ...
        {tasks.length > 0 && (
          ...
          >
            <AnimatePresence>
              {isHovering && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center text-blue-600">
